...
async function getAirdropStatus(address) {
  const apiUrl = `https://mdi-quests-api-production.up.railway.app/reward-claim/active-pools/status?walletAddress=${address}`;
  try {
    const response = await axios.get(apiUrl);
    const data = response.data;
    if (data !== undefined && data.length > 0) {
      const { amount } = data[0];
      console.log(`${address} has a claim: ${amount} MANTLE`);
      console.log(data);
    }
  } catch (error) {
